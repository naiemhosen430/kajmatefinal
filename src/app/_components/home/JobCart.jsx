...
export default function JobCart({data}) {
  ...
  return (
    ...
    <div onClick={() => set_job(data)} className="w-full cursor-pointer mx-auto p-4 bg-none border border-gray-500 my-2 rounded-lg shadow-md">
      {/* Cart Header (Title) */}
        <h2 className="text-2xl font-bold text-right text-white">{data?._doc?.profession}</h2>
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-xl font-semibold text-white">{data?._doc?.description}</h2>
      ...
      <div className="flex justify-between items-center text-gray-500 mb-4">
        <div className="flex text-gray-100 text-[10px] w-6/12 items-center">
          <span>{data?._doc?.area}</span>
        </div>
        <div className="flex text-gray-100 text-[10px] w-2/12 items-center">
          <span>{data?._doc?.status}</span>
        </div>
        <div className="flex text-gray-100 text-[10px] w-2/12 items-center">
          <span>{data?._doc?.need_type}</span>
        </div>
        <div className="flex text-gray-100 text-[10px] w-2/12 items-center">
          <span>{data?._doc?.type}</span>
        </div>
      </div>
